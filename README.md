...
| `END_TO_END` | **COVERED** | **RECORDED** — one correlated CONTRACT-PUBLISH run (capability-demo `2aac73e`, clean tree): one run_id, POINT 8 PROVEN, the readback names the governed contract's commit, and a signed `cr.exec.correlation.v1` that this suite re-verifies. Negative pole: the same producer refused an uncorrelated readback. | The contract-publish chain holds on one real change: decision → receipt → guarded execution → atomic commit → a provider observation correlated to the same commit. **NOT PATH B** — no pull request was merged under the grant, and the readback is witness-attested, not provider-signed. |
