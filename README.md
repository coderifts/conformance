...
| `END_TO_END` | **COVERED** | **RECORDED** — one target-state-transition run in which POINT 8 is filled from a BARE-GIT TARGET-STATE TRANSITION: a throwaway bare repository built inside the run, one ref update authorized by a signed `cr.exec.v2` grant, performed as a compare-and-swap, then read back by a SEPARATE PROCESS that cannot write and is never told what it is about to find. A signed `cr.evidence.root.v1` binds every token's exact digest to one `run_id`, which closes the `cross_run_collage` gap this row named until 0.8.5 — a genuinely signed token taken from a DIFFERENT run of the same producer is now refused by the root's digest. Four correlations are re-checked offline (after-state token, blob digest, content sha256, single parent), and the negative pole is the same producer refusing an uncorrelated readback. | **TRUSTED-EXECUTOR INTEGRITY, and no further.** `proof_scope: TRUSTED_EXECUTOR`, `provider_witness: NOT_APPLICABLE`, `externally_witnessed: false`. The executor and the observer are one machine and one OS user, separated by the target's mode bits — not by two identities and not by a third party. **NOT PATH B**: no pull request was merged and no provider witnessed anything. |

**PROFILE COVERAGE 7/7 · EVIDENCE 2 LIVE + 5 RECORDED + 0 MODELLED · OVERALL RECORDED · FULL LIVE false.**
