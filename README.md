...
| `END_TO_END` | **COVERED** | **RECORDED** — ONE run of the demo's contract-publish chain, captured from a clean clone at capability-demo `a39a407`: one run_id across ten points, POINT 8 PROVEN, the readback naming the governed contract's commit, a signed `cr.exec.correlation.v1` this suite rebuilds and re-verifies, and a negative pole where the same producer refused an uncorrelated readback. AUTHORIZATION IS CONTINUOUS: the run is challenge-first — the executor minted the state nonce, the live authorize bound `sha256(nonce)` into a server `cr.exec.v2` ATOMIC grant, and the executor consumed **that** grant, so issued, consumed and attested jti are one value and the correlation binds the scope the server signed. The measure re-checks each identity; it does not read the claim. | The chain is continuous end to end. **NOT PATH B**: no pull request was merged under the grant, and the readback is witness-attested, not provider-signed. |
...
- **`END_TO_END` — COVERED / RECORDED.** The two gaps this entry used to name are closed by measurement, not by relaxing the measure. `authorization_not_continuous` (server jti `d33032a5` vs executor jti `d26dbacc`) is gone because the capture is now challenge-first: a `cr.exec.v2` ATOMIC grant binds `sha256` of a nonce the executor itself minted, so the grant the server issued is the grant the executor can consume — and did. `readback_future` is gone because the vendored witness no longer postdates its own run. Both checks still run, and both still bite: a discontinuous or dirty-tree capture drops straight back to PARTIAL. What COVERED does **not** mean is that CodeRifts merged a pull request — that is PATH B, and the profile's last `does_not_prove` line says so.
...
node bin/coderifts-conformance.js --assurance END_TO_END        # exit 0 — COVERED (one continuous run)
```
