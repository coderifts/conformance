...
| `END_TO_END` | **PARTIAL** | **RECORDED** — the CONTRACT-COMMIT correlation is done and re-verified (capability-demo `2aac73e`, clean tree): one run_id, POINT 8 PROVEN, the readback names the governed contract's commit, a signed `cr.exec.correlation.v1` this suite rebuilds and checks, and a negative pole where the same producer refused an uncorrelated readback. What is NOT done is AUTHORIZATION CONTINUITY: the transcript carries two grants — the server issued `d33032a5`, the executor consumed `d26dbacc` — so the chain proves an authorize and an execution, not that the authorize covered that execution. The vendored witness also postdates its own run by 6 days and is refused (`readback_future`). | Contract-commit correlation: **done**. Authorization continuity: **not done**. **NOT PATH B** either way — no pull request was merged under the grant, and the readback is witness-attested, not provider-signed. |
...
- **`END_TO_END` — PARTIAL / RECORDED.** Two gaps, both named by the measure rather than described here: `authorization_not_continuous` (server jti `d33032a5` vs executor jti `d26dbacc` — one grant does not flow through authorize → consume → attestation → correlation) and `readback_future` (the witness observed 2026-09-12, six days after the run finished). The contract-commit half IS correlated and signature-checked; COVERED returns when the continuity checks pass, and the measure verifies that rather than being told.
