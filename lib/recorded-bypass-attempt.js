 ...
 *
 * 1370 named three claims (config-closure, gate-block, observed 405). 1395 keeps
 * the 405 *bytes* here (pin + parser) and scores them on PROVIDER_ENFORCED's
 * envelope: an admin merge attempt returned 405 naming the required context.
 *
 * This is a recorded artifact, not an eighth assurance profile. 1395: PROVIDER_ENFORCED
 * evaluate() attaches these bytes to its evidence-envelope (PR#4 failing = negative,
 * PR#5 expected = positive control). The pin and parser stay here so the 405 bodies
 * have one hash-verify home. COVERED on PROVIDER_ENFORCED now requires this pair.
 *
