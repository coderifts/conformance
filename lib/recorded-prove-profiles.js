...
const CREDENTIAL_DOES_NOT_PROVE = Object.freeze([
  ...
  'that a live kernel would refuse the same host INSERT today — this is one recorded run',
]);
...
const ATOMIC_DOES_NOT_PROVE = Object.freeze([
  ...
  // NOT DRIFT, and measured before it was reworded (1437). This line is about THIS profile's own
  // artifact — fixtures/recorded/prove-transcript, run prove-fb5c23bd — which genuinely records
  // POINT 8 MODELLED. END_TO_END reads a DIFFERENT artifact (fixtures/recorded/end-to-end) whose
  // POINT 8 is PROVEN with nothing MODELLED. Two profiles, two captures, both honest.
  //
  // What was wrong was the SENTENCE, not the claim: side by side in one table, "the artifact
  // records it MODELLED" reads as if it were about the run the reader just saw graded PROVEN. So
  // the artifact is named.
  'that POINT 8 merge happened — THIS profile\'s artifact (fixtures/recorded/prove-transcript, '
  + 'run prove-fb5c23bd) records it MODELLED. END_TO_END grades a different capture, in which '
  + 'POINT 8 is PROVEN; neither statement is about the other\'s evidence',
]);
