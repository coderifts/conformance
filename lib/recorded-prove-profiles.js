...
const CREDENTIAL_DOES_NOT_PROVE = Object.freeze([
  ...
  'that an unchanged-state read-back followed the denial — that byte is not in this bundle',
  'that POINT 3 is a live denial — it is a catalog posture receipt, not a mutation attempt',
]);

const ATOMIC_DOES_NOT_PROVE = Object.freeze([
  'that a stale state_token CAS was refused — that panel is not in this bundle',
  'that a consume-only (skip-seal / consumed_unsigned) path was observed — --skip-seal was not this run',
  'that a mutation-only path (mutation without consume) was refused',
  'that before/after row-count read-backs accompany the CAS claim',
...
function measureCredential(artifact, payload) {
  ...
  const unchangedReadback = Object.prototype.hasOwnProperty.call(ev, 'row_count')
    || Object.prototype.hasOwnProperty.call(ev, 'unchanged')
    || Object.prototype.hasOwnProperty.call(ev, 'after_count')
    || Object.prototype.hasOwnProperty.call(ev, 'articles_after');
  ...
  if (unchangedReadback) present.push('unchanged-state read-back');
  else missing.push('unchanged-state read-back after the denial (no row_count / after_count in deny.evidence)');
  if (point3IsCatalog) {
    missing.push('POINT 3 is catalog posture (cr.posture.receipt.v1), not a live host-mutation denial');
  }

  const earned = hostAttempt && targetDenial && notNode403;
  const coverage = earned && unchangedReadback ? COVERAGE.COVERED : (earned ? COVERAGE.PARTIAL : COVERAGE.NOT_COVERED);
...
function measureAtomic(artifact, payload) {
  ...
  const singleUse = rev.first === 201 && rev.second === 409 && rev.status === 'GRANT_CONSUMED';
  const concurrency = cev.ok === 1 && cev.grew === 1 && typeof cev.conflict === 'number' && cev.conflict >= 1;

  const staleCas = (payload.sections || []).some((s) => {
    const e = s.evidence || {};
    return /stale/i.test(s.id || '') || /stale/i.test(s.name || '')
      || Object.prototype.hasOwnProperty.call(e, 'stale_state_token')
      || Object.prototype.hasOwnProperty.call(e, 'expected_state_token');
  });
  const consumeOnly = (payload.sections || []).some((s) => {
    const e = s.evidence || {};
    return e.status === 'consumed_unsigned' || e.skip_seal === true || /skip-seal/i.test(s.id || '');
  });
  const mutationOnly = (payload.sections || []).some((s) => {
    const e = s.evidence || {};
    return e.mutation_only === true || e.unconsumed_mutation === true;
  });
  const readbacks = (payload.sections || []).some((s) => {
    const e = s.evidence || {};
    return Object.prototype.hasOwnProperty.call(e, 'before')
      || Object.prototype.hasOwnProperty.call(e, 'after')
      || Object.prototype.hasOwnProperty.call(e, 'before_count')
      || Object.prototype.hasOwnProperty.call(e, 'after_count');
  });
...
function evaluate(dir = FIXTURE_DIR) {
  ...
  const credEnv = makeEnvelope({
    ...
    extraBindings: [
      { name: 'modelled-merge-not-promoted', ok: !(p8 && p8.state === 'MODELLED' && credential.coverage === COVERAGE.COVERED), error: 'MODELLED merge must not make CREDENTIAL COVERED' },
    ],
  });
  const atomEnv = makeEnvelope({
    ...
    extraBindings: [
      { name: 'modelled-merge-not-promoted', ok: !(atomic.modelled_merge && atomic.coverage === COVERAGE.COVERED), error: 'MODELLED merge must not make ATOMIC COVERED' },
    ],
  });
