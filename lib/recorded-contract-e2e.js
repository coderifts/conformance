...
const E2E_CONTRACT_DOES_NOT_PROVE = Object.freeze([
  ...
  + 'authorized commit\'s parents are the ones the PRODUCER recorded, bound by the evidence root '
  + 'but not independently re-read from an object database (RECORDED, not re-measured)',
...
function measureContractE2E(opts = {}) {
  ...
  // re-derive them: it re-checks the parents the producer RECORDED. That is a weaker statement and
  // the difference is real — the producer's parent claim is bound by the evidence root (so it
  // cannot be edited afterwards) but it is not independently re-derived here. Recorded, not
  // re-measured, and the `does_not_prove` line below says so.
  ...
  if (tst) {
    ...
    const check = (id, ok, detail) => {
      ...
      else missing.push(`transition ${id}: ${detail}`);
      return ok;
    };
  ...
  const correlated = oneRun && transitionProven && hasFields && commitsAgree && signatureValid
  ...
  try {
    ...
    if (b.authorized_and_committed) {
      present.push(`authorization: ${b.state} (core predicate, shared with prove and the guard)`);
    } else {
