...
function measureContractE2E(opts = {}) {
  ...
  try {
    ...
    if (b.requirements_satisfied) {
      present.push(`authorization: ${b.state} — issuer_grant + one_run_root, a CUSTOM set `
        + '(the closed TRUSTED_EXECUTOR_INTEGRITY_V1 profile also requires the executor '
        + 'attestation, which needs one grant end-to-end)');
    } else {
