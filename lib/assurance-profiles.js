...
const PROFILES = Object.freeze([
  ...
  },
  {
    ...
    why_empty:
      ...
      + '(a fake host would imply coverage that does not exist). RECORDED from the capability-demo '
      + 'prove transcript: the DENY panel carries a real target-side 42501 on cr_host INSERT. '
      + 'POINT 3 is a catalog posture receipt, not that denial. Unchanged-state read-back is '
      + 'missing from the signed evidence — PARTIAL, not COVERED. Live mode without infra is '
      + 'NOT_RUN and does not fall back. does_not_prove is non-empty. Source comments in db.js '
      + 'are MODELLED and are not counted.',
  },
  {
    id: 'ATOMIC_COMMIT',
    ...
    why_empty:
      ...
      + 'capability-demo prove transcript: replay 201 then 409 GRANT_CONSUMED, and concurrency '
      + 'ok=1 grew=1. Missing from the signed bundle: stale state_token CAS, consume-only '
      + '(skip-seal), mutation-only, before/after read-backs. PARTIAL, not COVERED. db.js / '
      + 'server.js negatives are MODELLED source and are not counted. Live mode without infra '
      + 'is NOT_RUN and does not fall back.',
  },
  {
