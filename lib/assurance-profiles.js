...
const PROFILES = Object.freeze([
  ...
  },
  {
    ...
    does_not: null,
    why_empty:
      'RECORDED from raw GitHub dumps of coderifts/demo (ruleset 22074842, PR#4 required-context '
      + 'FAILURE+BLOCKED + PR#5 required-context SUCCESS, plus merge-API 405 bodies: PR#4 failing, '
      + 'PR#5 expected). ADV-1 remains DECISION_LOGIC — a local mirror is not a provider refusing '
      + 'anything. Capture is a local gh token dump, not a GitHub-signed object and not OIDC. '
      + 'Live mode without gh is NOT_RUN and does not fall back. does_not_prove is non-empty '
      + '(HISTORICAL, local-gh, PR#5 BEHIND).',
  },
  {
    id: 'END_TO_END',
    // ── THE TITLE AND THE ASSERTION NAME WHAT IS ACTUALLY MEASURED ──────────────────────
    //
    // They said "Authorization through to deploy" and "…atomic commit and provider enforcement"
    // while the measure grades a `git.ref.update` on a local bare-Git target with
    // `provider_witness: NOT_APPLICABLE`. No provider is involved and nothing is deployed. This is
    // the sibling of the does_not_prove fix: the same PATH-B-shaped wording, in the row a reader
    // meets FIRST — before any limit, and often instead of them.
    //
    // `titleFor` / `assertsFor` below derive these from the graded capture, so a provider-shaped
    // capture would get provider wording back rather than this one being find-and-replaced.
    title: 'Authorization through to a governed target state',
    asserts:
      'The whole chain holds together on one real change and ONE authorization: decision, receipt, '
      + 'guarded execution, atomic commit and a read-after-write of the governed target, in sequence.',
    does_not: null,
    why_empty:
      'The layers exist as separately recorded artifacts, but they are not one correlated run. '
      + 'The prove-transcript is a Postgres executor run whose POINT 8 merge is MODELLED; the '
      + 'PROVIDER_ENFORCED bundle is a GitHub observation of different commits on coderifts/demo. '
      + 'No shared run_id. A collage is layer-coverage, not end-to-end. PARTIAL, not COVERED.',
  },
]);
 ...
 *
 * UNIT DISTINCTION (not an aggregation bug):
 *   vectors_present  = unique vectors
 *   vectors_positive / vectors_negative = polarity OCCURRENCES
 * A `pair` vector is 1 present, +1 positive, +1 negative, so present need not
 * equal positive+negative. Overlay profiles may also report pair-completeness
 * counts (at-least-one of each pole) rather than unique-vector counts.
 */
...
/**
 * @param {string} mode  'recorded' | 'live'
 * @param {object} [opts]
 * @param {string} [opts.dir]  an EXTERNAL capture directory for END_TO_END. Threaded all the way
 *   down rather than read from the environment, so `--dir` is a real argument of the measurement
 *   and not a global a caller might forget to set.
 */
function recordedOverlay(mode, opts = {}) {
  ...
  if (evidence === 'live') {
    ...
    const provider = require('./recorded-provider-enforced');
    return {
      ...
      PROVIDER_ENFORCED: provider.liveUnavailable('PROVIDER_ENFORCED'),
      END_TO_END: provider.liveUnavailable('END_TO_END'),
    };
  }
  const cryptoRec = require('./recorded-receipt-crypto').evaluate();
  const proveRec = require('./recorded-prove-profiles').evaluate();
  const providerRec = require('./recorded-provider-enforced').evaluate();
  // END_TO_END now has TWO candidate measures, and the stronger one wins only if it is genuinely
  // correlated. They are different chains, not two readings of one:
  //
  //   contract-publish  ONE run — the demo's governed contract, a readback naming the same commit,
  //                     a signed correlation this measure RE-VERIFIES. COVERED when it verifies.
  //   prove x provider  a Postgres executor run correlated against a separate GitHub capture. No
  //                     shared run_id, no shared commit — a collage, and it stays PARTIAL. It is
  //                     kept, not deleted: it states a gap that is still real.
  //
  // The fallback is deliberate. If the vendored contract-publish run ever stops verifying, the
  // profile drops back to the honest PARTIAL rather than disappearing.
  const contractE2E = require('./recorded-contract-e2e')
    .measureContractE2E(opts.dir ? { dir: opts.dir } : {});
  const collageE2E = require('./recorded-provider-enforced').measureEndToEnd();
  // THE CONTRACT MEASURE WINS EITHER WAY, and that is a correction.
  //
  // The first version fell back to the collage whenever the contract run was not correlated — which
  // meant that the moment the continuity gate found the two-grant gap, the profile stopped
  // REPORTING it and showed the collage's older gaps instead. Both are PARTIAL, so the coverage was
  // unchanged; what was lost was the reason, which is the only part anyone can act on.
  //
  // ── WHOSE GAPS? A CORRECTION (1465) ─────────────────────────────────────────────────────
  //
  // The collage measure is kept and still runs. Its gaps used to be UNIONED into this profile's
  // `gaps` and `missing`, prefixed but present, on the reasoning that they were "carried alongside,
  // not instead". That reasoning held while BOTH measures were PARTIAL and the grade was the same
  // either way.
  //
  // MEASURED once the contract measure went COVERED: the profile reported
  //   coverage: COVERED   gaps: [3 entries]
  // — a covered profile carrying three named gaps. The three are real, and every one of them is
  // about a DIFFERENT capture (the prove x provider collage, which this profile does not grade).
  // A reader cannot act on that: either the grade is wrong or the gaps are, and nothing in the
  // output says which.
  //
  // `gaps` now means what a reader assumes it means — gaps in THIS profile's evidence. The
  // collage's gaps keep their own field and their own name, so they are still printed, still
  // true, and no longer contradicting a grade they were never about.
  const adjacent = collageE2E.gaps.map((g) => `[prove x provider collage] ${g}`);
  const e2eRec = {
    ...contractE2E,
    gaps: [...contractE2E.gaps],
    missing: [...contractE2E.missing],
    /** Gaps in a NEIGHBOURING capture this profile does not grade. Never a conjunct of the grade. */
...
function buildProfileReport(opts = {}) {
  ...
  const overlay = recordedOverlay(evidence, { dir: opts.dir });
  return PROFILES.map((p) => {
    ...
    if (over) {
      ...
      // WHICH BYTES. Threaded onto the row so the CLI can print it beside the verdict — the whole
      // point of `--dir` is that the answer names the capture it is about.
      if (over.evidence_dir) { evidenceDir = over.evidence_dir; source = over.source; }
      // THE HEADLINE FOLLOWS THE CAPTURE. A title kept in a static table describes whatever the
      // profile graded when someone last edited it — which is exactly how "deploy" and "provider
      // enforcement" outlived the capture they were written for.
      if (over.headline) { headline = over.headline; }
    ...
    const row = {
      ...
      does_not_prove,
      gaps,
      ...(evidenceDir ? { evidence_dir: evidenceDir, source } : {}),
      ...(headline ? { title: headline.title, asserts: headline.asserts } : {}),
      why_empty,
      vector_ids,
      evidence,
    };
...
/**
 * One-line unit docs for the count fields. present ≠ pos+neg is expected when a
 * vector is polarity `pair`, or when an overlay reports pair-completeness.
 */
const FIELD_DOCS = Object.freeze({
  vectors_present:
    'Unique vectors mapped to this profile (one VECTOR_MAP row, or overlay vector_ids). Not a polarity sum.',
  vectors_runnable:
    'Unique mapped vectors whose runner is not none.',
  vectors_positive:
    'Polarity occurrences that count as positive. A `pair` vector is +1 here AND +1 in vectors_negative. Overlay profiles may report pair-completeness (at-least-one of each pole), not unique-vector counts. vectors_present need not equal vectors_positive + vectors_negative.',
  vectors_negative:
    'Polarity occurrences that count as negative. See vectors_positive. A `pair` vector counts here as well as in vectors_positive.',
});
...
function renderProfileTable(rows = buildProfileReport()) {
  ...
  out.push('');
  out.push(
    '  counts: vectors_present = unique vectors; vectors_positive/negative = polarity occurrences '
    + '(a pair vector is +1 in both). present need not equal pos+neg. Overlay pair-completeness '
    + 'counts may also diverge.',
  );
  out.push('');
...
function renderProfileJson(rows = buildProfileReport()) {
  ...
  return {
    ...
    profiles: rows.map((r) => ({
      ...
      // ── `result` RENAMED, because it read as the answer ─────────────────────────────────
      //
      // MEASURED on the two-grant negative: `result: PASS` sat beside `status: PARTIAL`. `result`
      // meant "the vectors executed without erroring" — a statement about the MEASUREMENT — while
      // an external reader parsing one field reasonably takes `result` for the verdict. Two fields
      // in one object, one saying PASS and one saying the claim is unproven.
      //
      // `measurement_completed` cannot be mistaken for a claim, and `claim_status` cannot be
      // mistaken for a run outcome. `result` is kept as a deprecated alias for one release so a
      // consumer parsing it does not break silently — but it now carries the CLAIM's value, so a
      // reader who ignores this note still gets the honest answer.
      measurement_completed: r.result,
      claim_status: r.coverage || r.status,
      result: r.coverage || r.status,
      result_note: 'DEPRECATED alias of claim_status. `measurement_completed` is whether the '
        + 'vectors ran; `claim_status` is whether the claim is proven. They are different questions.',
      ...
      gaps: r.gaps || [],
      // Carried up so the CLI can name the bytes it graded without re-reading anything.
      ...(r.evidence_dir ? { evidence_dir: r.evidence_dir, source: r.source } : {}),
