...
function recordedOverlay(mode) {
  ...
  //
  // The collage measure is kept and still runs: its gap ("two runs, no shared run_id") is real and
  // is carried alongside, not instead.
  const e2eRec = {
    ...contractE2E,
    gaps: [...contractE2E.gaps, ...collageE2E.gaps.map((g) => `[prove x provider collage] ${g}`)],
    missing: [...contractE2E.missing, ...collageE2E.gaps.map((g) => `[prove x provider collage] ${g}`)],
  };
  return {
