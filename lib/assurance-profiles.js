...
const PROFILES = Object.freeze([
  ...
  },
  {
    ...
    why_empty:
      'RETIRED FROM THIS SUITE (0.4.0), and the row is KEPT so the claim is not silently '
      + 'dropped. The 23 EG-* / EG2-* / EG-A-* / MON-A-* vectors were vendored into '
      + 'cases.v1.json and NONE of them could execute here: every shipped subject implements '
      + 'only the `decide` and `tool_selection` kinds and throws `unknown case kind` on '
      + '`execution_grant`, `execution_attestation` and `monitoring_attestation`. Sitting '
      + 'outside the default profile, they never surfaced as failures — a third of the case '
      + 'file was data with no runner, which is exactly what one number hides. '
      + 'THE REMOVAL COULD NOT START HERE: cases.v1.json is vendored from the app and gated '
      + 'byte-identical by its test/conformance-cases-vendored-sync.test.js, so deleting rows '
      + 'here would have broken the app CI. The change began at the app-canonical copy and '
      + 'this repo followed it. '
      + 'WHERE THEY RUN NOW — both stronger than a runner here: in the APP against the real '
      + 'verify functions (test/execution-grant.test.js, test/execution-attestation.test.js, '
      + 'test/monitoring-attestation.test.js, over receipt-crypto-vectors.v1.json); and in '
      + 'receipt-verifier as SIGNED TOKEN BYTES cross-checked by two independent '
      + 'implementations, JS and Python. A runner here would have MINTED the token from the '
      + 'scenario name and then verified it — a generator and a verifier in one repository '
      + 'agreeing with itself, which measures nothing. '
      + 'NOT COVERED is the honest status FOR THIS SUITE. It is not a claim that the property '
      + 'is unproven; it is a statement that this repository is not where the proof lives.'
  },
  {
  ...
  },
  {
    ...
    why_empty:
      ...
      + 'kept because it is the reason a vector was never filed here: EG-A-STATE-NONCE-MISMATCH '
      + 'names a nonce and LOOKED like coverage, while it actually checks that an attestation '
      + 'document is unbound — a binding fact, not an atomicity one. Filing it here would have '
      + 'made this profile non-empty on false evidence. It left with the rest of RECEIPT_CRYPTO '
      + 'in 0.4.0 and now runs in the app and in receipt-verifier; this profile is unaffected, '
      + 'because it never counted here.',
  },
  {
...
/**
 * Status of a profile given its mapped vectors. Exported so a bite-test can
 * prove positives-alone are not COVERED without mutating the live map.
 */
function coverageStatus(vectors) {
  ...
  if (list.length === 0) return STATUS.NOT_COVERED;
  if (runnable.length === 0) return STATUS.NOT_RUN;
  if (!polarityCounts(runnable).complete) return STATUS.NOT_COVERED;
...
function buildProfileReport() {
  return PROFILES.map((p) => {
    ...
    const status = coverageStatus(vectors);
    let why_empty = p.why_empty || null;
    if (status === STATUS.NOT_COVERED && vectors.length > 0 && !polar.complete) {
    ...
    return {
      ...
      positive: polar.positive,
      negative: polar.negative,
      asserts: p.asserts,
...
function assertNoGreenEmpty(rows) {
  for (const r of rows) {
    ...
    if (r.status === STATUS.NOT_RUN && r.runnable > 0) {
...
function renderProfileTable(rows = buildProfileReport()) {
  ...
  for (const r of rows) {
    ...
    if (r.status === STATUS.COVERED) {
      evidence = `${r.runnable} vector(s) run (${r.positive} positive, ${r.negative} negative)`;
