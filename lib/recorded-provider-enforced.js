 ...
 *
 * 1395 — the observed merge-API 405 is ON this envelope (not a side artifact):
 *   negative — PR#4 PUT …/merge HTTP 405, required context "is failing"
 *   positive — PR#5 PUT …/merge HTTP 405, required context "is expected" (control)
 * Bytes live in fixtures/recorded/bypass-attempt (pin + parser). COVERED here
 * requires both check-poles AND that 405 pair. evidence_tier stays RECORDED.
 *
...
const BA = require('./recorded-bypass-attempt');
...
const DOES_NOT_PROVE = Object.freeze([
  ...
  'that PR#5\'s merge-API 405 is a gate-refusal — its reason is "expected" (branch BEHIND), not "failing"; it is the gate-specificity control beside PR#4\'s "failing" 405',
  ...
  'that a gate-SUCCESS + up-to-date merge was observed — it was not. The 405 pair is a merge-API refusal body (HISTORICAL, local gh), not that a merge proceeded',
  'that any merge has LANDED under enforcement — MEASURED 2026-09-05: zero commits reached main since the ruleset was created (2026-09-02T08:34Z). The 405 is a refused attempt, not a successful merge',
...
function evaluate(dir = FIXTURE_DIR) {
  ...
  const baPin = BA.assertPin();
  const pr4_405 = readJson(BA.FIXTURE_DIR, 'pr4-merge-refusal.json');
  const pr5_405 = readJson(BA.FIXTURE_DIR, 'pr5-merge-refusal.json');
  const refusals = BA.measureRefusals(pr4_405.doc, pr5_405.doc);
  const mergeOk = refusals.coverage === COVERAGE.COVERED;
  if (mergeOk) {
    measured.present.push(...refusals.present);
  } else {
    measured.missing.push(...refusals.missing);
  }
  const coverage = (measured.both_poles && mergeOk) ? COVERAGE.COVERED
    : ((measured.both_poles || mergeOk) ? COVERAGE.PARTIAL : COVERAGE.NOT_COVERED);
  ...
  const pr4_405Hash = sha256hex(pr4_405.bytes);
  const pr5_405Hash = sha256hex(pr5_405.bytes);
  const artifacts = [
    { role: 'positive', sha256: posHash },
    { role: 'negative', sha256: negHash },
    { role: 'negative', sha256: checksHash },
    { role: 'negative', sha256: pr4_405Hash },
    { role: 'positive', sha256: pr5_405Hash },
  ];
  ...
  assertValidEnvelope(envelope, {
    attached: [
      ...
      { role: 'negative', bytes: pr4_405.bytes, sha256: pr4_405Hash },
      { role: 'positive', bytes: pr5_405.bytes, sha256: pr5_405Hash },
    ],
    ...
    bindings: [
      ...
      {
        name: 'pr4-merge-api-405-failing',
        ok: refusals.refusals.pr4.status === '405'
          && refusals.refusals.pr4.reason === 'failing'
          && refusals.refusals.pr4.named,
        error: 'PR#4 merge-API 405 must name CodeRifts / contract-gate is failing',
      },
      {
        name: 'pr5-merge-api-405-expected',
        ok: refusals.refusals.pr5.status === '405'
          && refusals.refusals.pr5.reason === 'expected'
          && refusals.refusals.pr5.named,
        error: 'PR#5 merge-API 405 must name CodeRifts / contract-gate is expected (control)',
      },
      {
        name: 'two-reasons-gate-specificity',
        ok: refusals.refusals.pr4.reason !== refusals.refusals.pr5.reason,
        error: 'failing vs expected must differ — otherwise the 405 block is not shown to be gate-specific',
      },
      {
        name: '405-pin',
        ok: Boolean(baPin && baPin.provenance_from_artifact
          && baPin.provenance_from_artifact.oidc_attested === false),
        error: '405 capture provenance must stay local-gh, not OIDC',
      },
    ],
    operational: true,
  });

  const why = coverage === COVERAGE.COVERED
    ? null
