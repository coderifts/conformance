...
const DOES_NOT_PROVE = Object.freeze([
  ...
  'that a merge API 405 is THIS profile\'s evidence — the HTTP 405 bodies are a separate recorded artifact (fixtures/recorded/bypass-attempt / observed_bypass_failure), not scored into PROVIDER_ENFORCED coverage',
  ...
  // 1370 — three claims, not one. CONFIG-CLOSURE is this profile: bypass_actors:[] and no
  // classic branch protection. GATE-BLOCK is this profile's COVERED negative pole (PR#4
  // required-context FAILURE + BLOCKED). OBSERVED BYPASS-FAILURE is a different artifact
  // (fixtures/recorded/bypass-attempt) and is not scored here.
  'that a bypass was ATTEMPTED and failed as THIS profile\'s evidence — that observation is the bypass-attempt artifact (1370); this profile\'s negative pole remains the gate failing on PR#4. bypass_actors:[] here is still configuration, not that observation',
  'that any merge has LANDED under enforcement — MEASURED 2026-09-05: zero commits reached main since the ruleset was created (2026-09-02T08:34Z). A refused merge attempt is the bypass-attempt artifact, not a successful merge',
